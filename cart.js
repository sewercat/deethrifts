...
function getProductImageSrc(img, w, h) {
 const width = w || 360;
 const height = h || 360;
 if (isDirectImageSource(img)) return img;
 return `https://picsum.photos/seed/${encodeURIComponent(img || 'dee-fallback')}/${width}/${height}.jpg`;
}
...
const Customers = {
 ...
 console.warn('Customer profile check failed, assuming first-time:', e);
...
const Orders = {
 ...
 if (!this._url) return [];
 try {
 const res = await fetch(this._url + '?redirect=false&action=orders&status=pending');
 const data = await res.json();
